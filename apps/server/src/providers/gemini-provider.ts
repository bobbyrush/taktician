...
import { GEMINI_MODEL_MAP, type GeminiAuthStatus } from '@taktician/types';
import { createLogger, isAbortError } from '@taktician/utils';
import { spawnJSONLProcess, type SubprocessOptions } from '@taktician/platform';
...
export class GeminiProvider extends CliProvider {
  ...
  /**
   * Convert Gemini event to Taktician ProviderMessage format
   */
  ...
  private async ensureGeminiIgnore(cwd: string): Promise<void> {
    ...
    const content = [
      '# Auto-generated by Taktician to speed up Gemini CLI startup',
