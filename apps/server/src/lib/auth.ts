...
const SESSION_COOKIE_NAME = 'taktician_session';
...
function ensureApiKey(): string {
  ...
  if (process.env.TAKTICIAN_API_KEY) {
    logger.info('Using API key from environment variable');
    return process.env.TAKTICIAN_API_KEY;
  }
...
if (!isEnvTrue(process.env.TAKTICIAN_HIDE_API_KEY)) {
  const autoLoginEnabled = isEnvTrue(process.env.TAKTICIAN_AUTO_LOGIN);
  ...
  const line4 = `Auto-login (TAKTICIAN_AUTO_LOGIN): ${autoLoginStatus}`.padEnd(BOX_CONTENT_WIDTH);
  const tipHeader = '💡 Tips'.padEnd(BOX_CONTENT_WIDTH);
  const line5 = 'Set TAKTICIAN_API_KEY env var to use a fixed key'.padEnd(BOX_CONTENT_WIDTH);
  const line6 = 'Set TAKTICIAN_AUTO_LOGIN=true to skip the login prompt'.padEnd(BOX_CONTENT_WIDTH);
...
`);
  ...
  logger.info('API key banner hidden (TAKTICIAN_HIDE_API_KEY=true)');
}
...
export function authMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  if (isEnvTrue(process.env.TAKTICIAN_DISABLE_AUTH)) {
    next();
    return;
  }
...
export function getAuthStatus(): { enabled: boolean; method: string } {
  const disabled = isEnvTrue(process.env.TAKTICIAN_DISABLE_AUTH);
  return {
    enabled: !disabled,
...
export function isRequestAuthenticated(req: Request): boolean {
  if (isEnvTrue(process.env.TAKTICIAN_DISABLE_AUTH)) return true;
  const result = checkAuthentication(
    req.headers as Record<string, string | string[] | undefined>,
    req.query as Record<string, string | undefined>,
    (req.cookies || {}) as Record<string, string | undefined>
