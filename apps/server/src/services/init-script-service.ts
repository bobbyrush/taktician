 ...
 *
 * Runs the .taktician/worktree-init.sh script after worktree creation.
...
import { systemPathExists, getShellPaths, findGitBashPath } from '@taktician/platform';
...
export class InitScriptService {
  ...
  getInitScriptPath(projectPath: string): string {
    return path.join(projectPath, '.taktician', 'worktree-init.sh');
  }
  ...
  async runInitScript(options: InitScriptOptions): Promise<void> {
    ...
    const safeEnv: Record<string, string> = {
      // Taktician-specific variables
      TAKTICIAN_PROJECT_PATH: projectPath,
      TAKTICIAN_WORKTREE_PATH: worktreePath,
      TAKTICIAN_BRANCH: branch,
