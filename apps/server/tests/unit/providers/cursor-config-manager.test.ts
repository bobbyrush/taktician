...
vi.mock('@taktician/platform', () => ({
  getTakticianDir: vi.fn((projectPath: string) => path.join(projectPath, '.taktician')),
}));

describe('cursor-config-manager.ts', () => {
  // Use platform-agnostic paths
  const testProjectPath = path.join(os.tmpdir(), 'test-project');
  const expectedConfigPath = path.join(testProjectPath, '.taktician', 'cursor-config.json');
