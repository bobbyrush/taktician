...
/**
 * Note: auth.ts reads TAKTICIAN_API_KEY at module load time.
 * We need to reset modules and reimport for each test to get fresh state.
 */
describe('auth.ts', () => {
  beforeEach(() => {
    vi.resetModules();
    delete process.env.TAKTICIAN_API_KEY;
    delete process.env.TAKTICIAN_HIDE_API_KEY;
  ...
  describe('authMiddleware', () => {
    ...
    it('should reject request with invalid API key', async () => {
      process.env.TAKTICIAN_API_KEY = 'test-secret-key';
  ...
  describe('getSessionCookieName', () => {
    it('should return the session cookie name', async () => {
      ...
      expect(name).toBe('taktician_session');
    });
  });
