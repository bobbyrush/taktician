...
export async function authenticateWithApiKey(page: Page, apiKey: string): Promise<boolean> {
  try {
    ...
    if (response?.success && response.token) {
      ...
      // The server sets a cookie named 'taktician_session' (see SESSION_COOKIE_NAME in auth.ts)
      await page.context().addCookies([
        {
          name: 'taktician_session',
 ...
 * Falls back to a test default if TAKTICIAN_API_KEY is not set
 */
export async function authenticateForTests(page: Page): Promise<boolean> {
  // Use the API key from environment, or a test default
  const apiKey = process.env.TAKTICIAN_API_KEY || 'test-api-key-for-e2e-tests';
