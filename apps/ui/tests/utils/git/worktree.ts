...
export async function createTestGitRepo(tempDir: string): Promise<TestRepo> {
  ...
  // Create .taktician directories
  const takticianDir = path.join(tmpDir, '.taktician');
  const featuresDir = path.join(takticianDir, 'features');
  fs.mkdirSync(featuresDir, { recursive: true });

  // Create empty categories.json to avoid ENOENT errors in tests
  fs.writeFileSync(path.join(takticianDir, 'categories.json'), '[]');
...
export function readTestFeature(repoPath: string, featureId: string): FeatureData | null {
  const featureFilePath = path.join(repoPath, '.taktician', 'features', featureId, 'feature.json');
