...
/**
 * Gets the default Documents/Taktician directory path
 * @returns Promise resolving to Documents/Taktician path, or null if unavailable
 */
async function getDefaultDocumentsPath(): Promise<string | null> {
  try {
    ...
    if (typeof window !== 'undefined' && window.electronAPI?.getPath) {
      ...
      return joinPath(documentsPath, 'Taktician');
    }
 ...
 * 3. Documents/Taktician (if ALLOWED_ROOT_DIRECTORY is not set)
...
export async function getDefaultWorkspaceDirectory(): Promise<string | null> {
  try {
    ...
    if (result.success) {
      ...
      // Try to get Documents/Taktician
