 ...
 *
 * Handles the setup of the .taktician directory structure when opening
...
/**
 * Required files and directories in the .taktician directory
...
const REQUIRED_STRUCTURE: {
  ...
  directories: ['.taktician', '.taktician/context', '.taktician/features', '.taktician/images'],
  files: {
    '.taktician/categories.json': '[]',
  },
};

/**
 * Initializes the .taktician directory structure for a project
 *
...
/**
 * Checks if a project has the required .taktician structure
 *
 * @param projectPath - The root path of the project
...
export async function hasAppSpec(projectPath: string): Promise<boolean> {
  const api = getElectronAPI();
  try {
    const fullPath = `${projectPath}/.taktician/app_spec.txt`;
...
/**
 * Checks if the .taktician directory exists for a project
 *
 * @param projectPath - The root path of the project
 * @returns true if .taktician directory exists
 */
export async function hasTakticianDir(projectPath: string): Promise<boolean> {
  const api = getElectronAPI();
  try {
    const fullPath = `${projectPath}/.taktician`;
    return await api.exists(fullPath);
  } catch (error) {
    logger.error('Error checking .taktician dir:', error);
    return false;
  }
}
