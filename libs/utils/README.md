...
This package provides common utility functions used across Taktician's server and UI. It includes error handling, logging, conversation utilities, image handling, and prompt building.

## Installation

```bash
npm install @taktician/utils
```
...
```typescript
import { createLogger, LogLevel } from '@taktician/utils';
...
```typescript
import { buildPromptWithImages } from '@taktician/utils';
...
```typescript
import { ensureDir, fileExists, readJsonFile, writeJsonFile } from '@taktician/utils';
...
```typescript
import { createLogger, classifyError, buildPromptWithImages } from '@taktician/utils';
