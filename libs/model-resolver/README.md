...
```bash
npm install @taktician/model-resolver
```
...
import { CLAUDE_MODEL_MAP } from '@taktician/types';
...
```typescript
import { getEffectiveModel } from '@taktician/model-resolver';
...
```typescript
import { DEFAULT_MODELS } from '@taktician/model-resolver';
import { CLAUDE_MODEL_MAP } from '@taktician/types';
...
- `@taktician/types` - Model type definitions and constants

## Used By

- `@taktician/server` - Feature execution, agent chat, enhancement
