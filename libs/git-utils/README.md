...
Git operations and utilities for Taktician.
...
```bash
npm install @taktician/git-utils
```
...
```typescript
import { parseGitStatus } from '@taktician/git-utils';
import type { FileStatus } from '@taktician/git-utils';
...
```typescript
import { listAllFilesInDirectory, generateDiffsForNonGitDirectory } from '@taktician/git-utils';
...
```typescript
import { isGitRepo, getGitRepositoryDiffs, parseGitStatus } from '@taktician/git-utils';
...
try {
  ...
  // Errors are logged via @taktician/utils logger
...
- `@taktician/types` - FileStatus type definition
- `@taktician/utils` - Logger utilities

## Used By

- `@taktician/server` - Git routes, worktree operations, feature context
