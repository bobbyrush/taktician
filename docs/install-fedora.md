# Installing Taktician on Fedora/RHEL

This guide covers installation of Taktician on Fedora, RHEL, Rocky Linux, AlmaLinux, and other RPM-based distributions.
...
1. Visit [GitHub Releases](https://github.com/bobbyrush/taktician/releases)
2. Find the latest release and download the `.rpm` file:
   - Download: `Taktician-<version>-x86_64.rpm`
...
Install from GitHub releases URL without downloading first. Visit [releases page](https://github.com/bobbyrush/taktician/releases) to find the latest version.

**Fedora:**

```bash
# Replace v0.11.0 with the actual latest version
sudo dnf install https://github.com/bobbyrush/taktician/releases/download/v0.11.0/Taktician-0.11.0-x86_64.rpm
```
...
```bash
# Replace v0.11.0 with the actual latest version
sudo yum install https://github.com/bobbyrush/taktician/releases/download/v0.11.0/Taktician-0.11.0-x86_64.rpm
```

## Running Taktician

After successful installation, launch Taktician:

### From Application Menu

- Open Activities/Applications
- Search for "Taktician"
- Click to launch
...
Or create `~/.config/taktician/.env`:
...
Taktician stores configuration and cache in:

```
~/.taktician/                # Project-specific data
~/.config/taktician/         # Application configuration
~/.cache/taktician/          # Cache and temporary files
```
...
```bash
cat /usr/share/applications/taktician.desktop
```
...
If Taktician fails on SELinux-enforced systems:
...
# Check for denials
sudo ausearch -m avc -ts recent | grep taktician
...
Create custom SELinux policy based on ausearch output. For support, see [GitHub Issues](https://github.com/bobbyrush/taktician/issues).

### Port Conflicts

Taktician uses port 3008 for the internal server. If port is already in use:
...
Or configure Taktician to use different port (see Configuration section).
...
Taktician uses Chromium for rendering. GPU acceleration should work automatically on supported systems.
...
```bash
DISABLE_GPU_ACCELERATION=1 taktician
```
...
2. Check process: `ps aux | grep taktician`
3. Force quit if necessary: `killall taktician`
...
```bash
sudo dnf remove taktician
```

**RHEL/CentOS:**

```bash
sudo yum remove taktician
```
...
rm -rf ~/.config/taktician

# Remove cache
rm -rf ~/.cache/taktician
```
...
To build Taktician from source on Fedora/RHEL:
...
**Automatic Updates:**
Taktician checks for updates on startup. Install available updates through notifications.

**Manual Update:**

```bash
# Fedora
sudo dnf update taktician

# RHEL/CentOS
sudo yum update taktician

# Or reinstall latest release
sudo dnf remove taktician

# Download the latest .rpm from releases page
# https://github.com/bobbyrush/taktician/releases
# Then reinstall with:
# sudo dnf install ./Taktician-<VERSION>-x86_64.rpm
```
...
- [GitHub Issues](https://github.com/bobbyrush/taktician/issues) - Bug reports & feature requests
- [Discussions](https://github.com/bobbyrush/taktician/discussions) - Questions & community
...
taktician 2>&1 | tee taktician.log

# SELinux status
getenforce

# Relevant system logs
sudo journalctl -xeu taktician.service (if systemd service exists)
```
...
echo "ANTHROPIC_API_KEY=sk-ant-..." > ~/.config/taktician/.env
...
1. Generate policy from audit logs: `ausearch -m avc -ts recent | grep taktician`
...
```bash
chmod 600 ~/.config/taktician/.env
chmod 700 ~/.taktician/
chmod 700 ~/.config/taktician/
```
