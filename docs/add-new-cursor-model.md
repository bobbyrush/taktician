# Adding New Cursor Models to Taktician

This guide explains how to add new Cursor CLI models to Taktician. The process involves updating a single file with automatic propagation to the UI.
...
```bash
npm run build -w @taktician/types
```
...
- [ ] Rebuilt types package (`npm run build -w @taktician/types`)
