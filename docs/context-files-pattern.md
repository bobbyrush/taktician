...
This document describes how context files work in Taktician and how to use them in agent prompts.

## Overview

Context files are user-defined documents stored in `.taktician/context/` that provide project-specific rules, conventions, and guidelines for AI agents. They are automatically loaded and prepended to agent prompts.

## Directory Structure

```
{projectPath}/.taktician/context/
...
The `loadContextFiles` function from `@taktician/utils` provides a unified way to load context files:

```typescript
import { loadContextFiles } from '@taktician/utils';
...
**Path:** `/path/to/project/.taktician/context/CLAUDE.md`
...
**Path:** `/path/to/project/.taktician/context/CODE_QUALITY.md`
