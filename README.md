<p align="center">
  <img src="apps/ui/public/readme_logo.svg" alt="Taktician Logo" height="80" />
</p>
...
>
> Taktician itself was built by a group of engineers using AI and agentic coding techniques to build features faster than ever. By leveraging tools like Cursor IDE and Claude Code CLI, the team orchestrated AI agents to implement complex functionality in days instead of weeks.
>
> **Learn how:** Master these same techniques and workflows in the [Agentic Jumpstart course](https://agenticjumpstart.com/?utm=taktician-gh).
...
- [What Makes Taktician Different?](#what-makes-taktician-different)
...
Taktician is an autonomous AI development studio that transforms how you build software. Instead of manually writing every line of code, you describe features on a Kanban board and watch as AI agents powered by Claude Agent SDK automatically implement them. Built with React, Vite, Electron, and Express, Taktician provides a complete workflow for managing AI agents through a desktop application (or web browser), with features like real-time streaming, git worktree isolation, plan approval, and multi-agent task execution.

![Taktician UI](https://i.imgur.com/jdwKydM.png)

## What Makes Taktician Different?

Traditional development tools help you write code. Taktician helps you **orchestrate AI agents** to build entire features autonomously. Think of it as having a team of AI developers working for you—you define what needs to be built, and Taktician handles the implementation.

### The Workflow

1. **Add Features** - Describe features you want built (with text, images, or screenshots)
2. **Move to "In Progress"** - Taktician automatically assigns an AI agent to implement the feature
...
Taktician leverages the [Claude Agent SDK](https://www.npmjs.com/package/@anthropic-ai/claude-agent-sdk) to give AI agents full access to your codebase. Agents can read files, write code, execute commands, run tests, and make git commits—all while working in isolated git worktrees to keep your main branch safe. The SDK provides autonomous AI agents that can use tools, make decisions, and complete complex multi-step tasks without constant human intervention.

### Why This Matters

The future of software development is **agentic coding**—where developers become architects directing AI agents rather than manual coders. Taktician puts this future in your hands today, letting you experience what it's like to build software 10x faster with AI agents handling the implementation while you focus on architecture and business logic.
...
- 🛠️ Get help setting up or extending Taktician
...
- **[Claude Code CLI](https://code.claude.com/docs/en/overview)** - Install and authenticate with your Anthropic subscription. Taktician integrates with your authenticated Claude Code CLI to access Claude models.

### Quick Start

```bash
# 1. Clone the repository
git clone https://github.com/bobbyrush/taktician.git
cd taktician
...
**Authentication:** Taktician integrates with your authenticated Claude Code CLI. Make sure you have [installed and authenticated](https://code.claude.com/docs/en/quickstart) the Claude Code CLI before running Taktician. Your CLI credentials will be detected automatically.
...
Start Taktician in development mode:
...
./start-taktician.sh web          # Web browser
./start-taktician.sh electron     # Desktop app
./start-taktician.sh electron-debug  # Desktop + DevTools

# Additional options
./start-taktician.sh --help       # Show all available options
./start-taktician.sh --version    # Show version information
./start-taktician.sh --check-deps # Verify project dependencies
./start-taktician.sh --no-colors  # Disable colored output
./start-taktician.sh --no-history # Don't remember last choice
```
...
**History File:**
Your last selected mode is saved in `~/.taktician_launcher_history` for quick re-runs.
...
wget https://github.com/bobbyrush/taktician/releases/latest/download/Taktician-<version>-x86_64.rpm

# Install with dnf (Fedora)
sudo dnf install ./Taktician-<version>-x86_64.rpm

# Or with yum (RHEL/CentOS)
sudo yum localinstall ./Taktician-<version>-x86_64.rpm
```

#### Docker Deployment

Docker provides the most secure way to run Taktician by isolating it from your host filesystem.
...
Taktician integrates with your authenticated Claude Code CLI. To use CLI authentication in Docker, mount your Claude CLI config directory (see [Claude CLI Authentication](#claude-cli-authentication) below).
...
services:
  server:
    volumes:
      # Linux/macOS
      - ~/.claude:/home/taktician/.claude
      # Windows
      - C:/Users/YourName/.claude:/home/taktician/.claude
```
...
> ```
>
> Create a `.env` file in the taktician directory:
>
> ```
> UID=1000
> GID=1000
...
services:
  server:
    volumes:
      ...
      # Linux/macOS
      - ~/.config/gh:/home/taktician/.config/gh
      # Windows
      - 'C:/Users/YourName/AppData/Roaming/GitHub CLI:/home/taktician/.config/gh'

      # Mount git config for user identity (name, email)
      - ~/.gitconfig:/home/taktician/.gitconfig:ro
    environment:
...
docker exec --user taktician -w /app taktician-server npx playwright install chromium
```
...
services:
  server:
    volumes:
      - playwright-cache:/home/taktician/.cache/ms-playwright

volumes:
  playwright-cache:
    name: taktician-playwright-cache
```
...
- Mock agent mode available in CI with `TAKTICIAN_MOCK_AGENT=true`
...
- `TAKTICIAN_API_KEY` - Optional API authentication for the server
...
- `TAKTICIAN_SKIP_SANDBOX_WARNING` - Skip sandbox warning dialog (useful for dev/CI)
- `TAKTICIAN_AUTO_LOGIN=true` - Skip login prompt in development (ignored when NODE_ENV=production)

### Authentication Setup

Taktician integrates with your authenticated Claude Code CLI and uses your Anthropic subscription.
...
Once authenticated, Taktician will automatically detect and use your CLI credentials. No additional configuration needed!
...
- 💾 **Auto-save** - All work automatically persisted to `.taktician/` directory
...
- **@taktician/types** - Shared TypeScript definitions
- **@taktician/utils** - Logging, error handling, image processing
- **@taktician/prompts** - AI prompt templates
- **@taktician/platform** - Path management and security
- **@taktician/model-resolver** - Claude model alias resolution
- **@taktician/dependency-resolver** - Feature dependency ordering
- **@taktician/git-utils** - Git operations and worktree management

## Available Views

Taktician provides several specialized views accessible via the sidebar or keyboard shortcuts:
...
Taktician is built as an npm workspace monorepo with two main applications and seven shared packages:
...
- **File-Based Storage** - No database; features stored as JSON files in `.taktician/` directory
...
Taktician uses a file-based storage system (no database required):

#### Per-Project Data

Stored in `{projectPath}/.taktician/`:
...
>
> **We do not recommend running Taktician directly on your local computer** due to the risk of AI agents having access to your entire file system. Please sandbox this application using Docker or a virtual machine.
>
...
- [Contributing Guide](./CONTRIBUTING.md) - How to contribute to Taktician
